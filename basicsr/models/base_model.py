...
class BaseModel():
    ...
    def save(self, epoch, current_iter, remove_old=False):
        """Save networks and training state.
        
        Args:
            epoch (int): Current epoch.
            current_iter (int): Current iteration.
            remove_old (bool): Whether to remove old model and state files.
        """
        pass
    ...
    @master_only
    def save_network(self, net, net_label, current_iter, param_key='params', remove_old=False):
        """Save networks.
        ...
        Args:
            ...
            param_key (str | list[str]): The parameter key(s) to save network.
                Default: 'params'.
            remove_old (bool): Whether to remove old model files.
                Default: False.
        """
        if current_iter == -1:
            current_iter = 'latest'
        ...
        # 移除旧的模型文件
        if remove_old and str(current_iter) not in ['latest', 'best', '-1']:
            import glob
            old_files = glob.glob(os.path.join(self.opt['path']['models'], f'{net_label}_*.pth'))
            for old_file in old_files:
                # 跳过 best 和 last 文件
                if 'best' in old_file or 'last' in old_file or 'latest' in old_file:
                    continue
                # 跳过当前要保存的文件
                if save_filename in old_file:
                    continue
                try:
                    os.remove(old_file)
                    logger.info(f'Removed old model file: {old_file}')
                except Exception as e:
                    logger.warning(f'Failed to remove old model file {old_file}: {e}')
        ...
        # 同时保存为 last.pth 作为备份
        last_save_filename = f'{net_label}_last.pth'
        last_save_path = os.path.join(self.opt['path']['models'], last_save_filename)
    ...
    @master_only
    def save_training_state(self, epoch, current_iter, remove_old=False):
        ...
        Args:
            epoch (int): Current epoch.
            current_iter (int): Current iteration.
            remove_old (bool): Whether to remove old state files.
                Default: False.
        """
        state = {
            'epoch': epoch,
        ...
        if remove_old and str(current_iter) not in ['latest', 'best', '-1']:
            import glob
            old_files = glob.glob(os.path.join(self.opt['path']['training_states'], '*.state'))
            for old_file in old_files:
                # 跳过 best 和 last 文件
                if 'best' in old_file or 'last' in old_file or 'latest' in old_file:
                    continue
                ...
                try:
                    os.remove(old_file)
                    logger.info(f'Removed old state file: {old_file}')
                except Exception as e:
                    logger.warning(f'Failed to remove old state file {old_file}: {e}')
        
        torch.save(state, save_path)
        
        # 同时保存为 last.state 作为备份
        last_save_path = os.path.join(self.opt['path']['training_states'], 'last.state')
