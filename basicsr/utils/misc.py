    ...
    if opt['path']['resume_state']:
        ...
        for network in networks:
            ...
            if opt['path'].get('ignore_resume_networks') is None or (
                ...
                if resume_iter == 0:
                    # 如果resume_iter为0，优先尝试加载last模型
                    last_model_path = osp.join(opt['path']['models'], f'net_{basename}_last.pth')
                    if os.path.exists(last_model_path):
                        opt['path'][name] = last_model_path
                        logger.info(f"Set {name} to {last_model_path}")
                    else:
                        # 如果last模型不存在，尝试加载best模型
                        best_model_path = osp.join(opt['path']['models'], f'net_{basename}_best.pth')
                        if os.path.exists(best_model_path):
                            opt['path'][name] = best_model_path
                            logger.info(f"Set {name} to {best_model_path}")
                        else:
                            # 如果best模型也不存在，使用原始的pretrain_network路径
                            logger.info(f"Resume iter is 0, using original pretrain_network path: {opt['path'].get(name)}")
                else:
